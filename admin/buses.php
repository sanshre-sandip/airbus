        ...
        <aside class="w-64 bg-blue-700 text-white flex flex-col">
            ...
            <div class="p-4 border-t border-blue-500">
                <a href="../backend/logout.php"
                    class="block text-center bg-red-500 py-2 rounded hover:bg-red-600">Logout</a>
            </div>
        </aside>
        ...
        <main class="flex-1 p-8">
            <div class="flex justify-between items-center mb-8">
                ...
                <button onclick="document.getElementById('addModal').classList.remove('hidden')"
                    class="bg-blue-700 text-white px-4 py-2 rounded hover:bg-blue-800">+ Add New Bus</button>
            </div>
            ...
            <div class="bg-white rounded-lg shadow overflow-x-auto">
                <table class="min-w-full text-left border">
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="text-center py-4 text-gray-600">No buses available.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
                ...
                <div>
                    ...
                    <select name="route_id" required class="w-full p-2 border rounded focus:outline-blue-500">
                        ...
                        <?php while ($r = $routes->fetch_assoc()): ?>
                            <option value="<?= $r['id'] ?>">
                                <?= htmlspecialchars($r['from_location'] . " → " . $r['to_location']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div>
                    <label class="block text-gray-700 mb-2">Total Seats</label>
                    <input type="number" name="total_seats" min="10" required
                        class="w-full p-2 border rounded focus:outline-blue-500">
                </div>
                <div class="flex justify-between mt-6">
                    <button type="button" onclick="document.getElementById('addModal').classList.add('hidden')"
                        class="bg-gray-300 px-6 py-2 rounded hover:bg-gray-400">Cancel</button>
                    <button type="submit" name="add_bus"
                        class="bg-blue-700 text-white px-6 py-2 rounded hover:bg-blue-800">Add Bus</button>
                </div>
            </form>
        </div>
    </div>
